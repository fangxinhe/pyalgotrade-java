...
public class CSVFileUtil {
    ...
    public CSVFileUtil(String filename) throws Exception {
        ...
        headers = fromCSVLinetoArray(headerLine);
    ...
    public Map<String, String> getLineDict() throws Exception {
        String line = null;
        line = readLine();
        if (line == null)
            return null;
        List<String> array = fromCSVLinetoArray(line);
        int len = Math.min(array.size(), headers.size());
        Map<String,String> ret = new HashMap<>();
        for (int i = 0; i < len; i++) {
            ret.put(headers.get(i),array.get(i));
        }
        return ret;
    }
